...
public class TestSample {
	...
	public void testCategory() {
		Category parent = Category.builder().name("영화").build();
		Category actionMovie = Category.builder().name("액션").build();
		Category romanceMovie = Category.builder().name("로맨스").build();
		Category comedyMovie = Category.builder().name("코메디").build();
	...
	public void testItem() {
		Album album = Album.builder().name("화양연화")
									 .price(20000)
									 .stockQuantity(450)
									 .artist("bts").build();
		entityManager.persist(album);
		Book book = Book.builder().name("노인과 바다").price(10000).stockQuantity(200).author("헤밍웨이").isbn("12200-10").build();
